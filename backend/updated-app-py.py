...
#             print(f"Returning personalized result for {request.url}: {response['is_phishing']}")  # Debug
...
#     return {"message": "URL tagged successfu lly", "personalized": 1}

# @app.post('/untag-url')
# async def untag_url_endpoint(request: UntagURLRequest):
#     result = untag_url(request.url, request.user_id)
#     if not result:
#         raise HTTPException(status_code=404, detail="URL not found or no34 user tag exists")
...
class EnhancedJobDetailsRequest(BaseModel):
    job_id: str
    job_title: str
    company_name: str
...
class EnhancedCompanyInfoRequest(BaseModel):
    company_name: str
    company_id: Optional[str] = None
    user_id: Optional[str] = None

class EnhancedRecruiterInfoRequest(BaseModel):
    recruiter_name: str
